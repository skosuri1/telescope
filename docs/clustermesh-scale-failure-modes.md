...
| `aks_node_resource_group_lease_expired` | AKS control planes still report `Succeeded/Running`, but every declared `nodeResourceGroup` is absent and only stale NotReady Nodes remain | false (managed infrastructure is gone) | n/a | validate every managed RG before resume, extend managed-RG leases before the parent lease, and require a fresh cluster rebuild if any managed RG is missing | 77314 |
