...
@pytest.mark.parametrize("changes", [
    ...
    {"resume_build_id": 80011}, {"resume_build_id": "80015"},
])
...
def test_monitoring_publishes_readonly_plan_before_bounded_nonretrying_execution():
    ...
    checkpoint = next(row["inputs"] for row in steps
                      if row.get("displayName") == "Download completed monitoring actions for read-only finalization")
...
@pytest.mark.parametrize("fault,count", [
    ...
    ("mutating-completion", 2), ("journal-changed", 2), ("replayed-operation", 2),
])
def test_monitoring_freezes_full_retirement_tree_and_cleans_private_credentials(tmp_path, fault, count):
    ...
    for path in (source, checkpoint, checkout, private, binaries):
        path.mkdir()
    (checkpoint / "recovery.json").write_text('{"source_build":80015}', encoding="utf-8")
    ...
    helper.write_text(textwrap.dedent(
        ...
        assert value("--resume-build-id") == "80015"
        checkpoint = Path(value("--resume-directory"))
        assert json.loads((checkpoint / "recovery.json").read_text())["source_build"] == 80015
        ...
        output.write_text(json.dumps({
            "execute": execute, "mutation_started": fault == "mutating-plan" or (execute and fault == "mutating-completion"),
            "plan_valid": fault != "unsafe-plan", "success": execute,
            "completion_only": True, "journal_unchanged": fault != "journal-changed",
            "provider_actions_replayed": fault == "replayed-operation",
    ...
    environment = {
        **os.environ, "PATH": f"{binaries}:{os.environ['PATH']}", "PHASE": "plan",
        ...
        "RESUME_BUILD_ID": "80015", "RESUME_DIRECTORY": str(checkpoint),
