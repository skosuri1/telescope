...
def test_restart_mode_cannot_fall_through_to_other_jobs():
    ...
    key = "${{ if and(eq(parameters.scaleDebugQualifiedWorkerRetirementBuildId, 0), eq(parameters.scaleDebugCapacityQualificationBuildId, 0), eq(parameters.scaleDebugCapacityFirstRecoveryBuildId, 0), ne(parameters.scaleDebugRetainedWorkerRestartBuildId, 0)) }}"
    ...
    for row in stage["jobs"]:
        ...
        if condition.startswith("${{") and condition not in (
            key, "${{ if and(eq(parameters.scaleDebugQualifiedWorkerRetirementBuildId, 0), eq(parameters.scaleDebugCapacityQualificationBuildId, 0), ne(parameters.scaleDebugCapacityFirstRecoveryBuildId, 0)) }}",
            "${{ if and(eq(parameters.scaleDebugQualifiedWorkerRetirementBuildId, 0), ne(parameters.scaleDebugCapacityQualificationBuildId, 0)) }}",
            "${{ if ne(parameters.scaleDebugQualifiedWorkerRetirementBuildId, 0) }}",
        ):
