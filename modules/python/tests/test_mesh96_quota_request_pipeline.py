...
def test_quota_mode_is_exclusive_and_does_not_allocate_capacity():
    ...
    key = "${{ if and(eq(parameters.scaleDebugCapacityQualificationBuildId, 0), eq(parameters.scaleDebugCapacityFirstRecoveryBuildId, 0), eq(parameters.scaleDebugRetainedWorkerRestartBuildId, 0), or(eq(parameters.scaleDebugDv3QuotaRequestLimit, 5500), ne(parameters.scaleDebugQuotaRequestReceiptBuildId, 0))) }}"
